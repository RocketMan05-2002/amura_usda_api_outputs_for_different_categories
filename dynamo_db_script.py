...
# Input folders containing the raw USDA FDC API JSON outputs
SOURCE_FOLDERS = [
...
# (nutrient_id, nutrient_name, unit, nutrient_nbr, parent_id, nesting_level, group)
# Kept exactly as in transform_nutrients.py
HIERARCHY = [
    # Proximates
...
def build_parent_lookup() -> dict:
    """Build a lookup to easily get parent ID and Name given a nutrient_id."""
    # First, build a lookup of id -> name
    id_to_name = {entry[0]: entry[1] for entry in HIERARCHY}
    
    id_to_parent_info = {}
    for entry in HIERARCHY:
        nid, name, unit, nbr, parent_id, level, group = entry
        
        # Determine inheritance path recursively
        current_parent_id = parent_id
        parent_name = None
        
        if current_parent_id is not None:
             parent_name = id_to_name.get(current_parent_id)
             
        id_to_parent_info[nid] = {
            "parent_id": current_parent_id,
            "parent_name": parent_name
        }
        
    return id_to_parent_info
...
def process_file(src_path: Path, dst_folder: Path):
    """
    Process a raw JSON into the final fully flat representation.
    Extracts raw structure, references hierarchy, pulls correct values,
    then adds to dictionary with direct root keys representing flattened elements.
    """
    ...
    for entry in HIERARCHY:
        ...
        # Check if the food specifically contains this nutrient 
        # Or if it's just a category header - we only care if it has an amount
        fn_data = nutrient_lookup.get(nid)
        
        # We also need to check if ANY of its children have an amount or data
        # In the original transform_nutrients.py, a node was kept if it had data OR if any child had data.
        def has_data_or_children_with_data(current_nid):
            # The node has data just by existing in the API response (even without amount like categories)
            if current_nid in nutrient_lookup:
                return True
            # Check children
            for child_entry in HIERARCHY:
                if child_entry[4] == current_nid: # parent_id check
    ...
    # Just to be safe, if we previously modified the name and it already contains fdc_id at end
    if f"_{fdc_id}" in original_stem:
        dst_filename = f"{original_stem}.json"

    out_path = dst_folder / dst_filename
...
def main():
    ...
    for folder_name in SOURCE_FOLDERS:
        ...
        dst = script_dir / f"resolved_backend_json_final_backend_response_{folder_name}"
        ...
        for json_file in json_files:
            try:
                fname = process_file(json_file, dst)
                count += 1
    ...
    print("✅  Done. All flattened backend response files generated directly from raw inputs!")
